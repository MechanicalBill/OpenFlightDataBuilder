...
namespace AGNES
{
    class Program
    {
        ...
        private static void WriteClusteringDiagramToFile(Cluster cluster)
        {
            ...
            /*
            int numberOfFlights = DataPoints.Count();
            int maxFlightId = 0;
            double maxLevel = 0;

                        foreach(var point in DataPoints)
                        {
                            if(point.FlightId > maxFlightId)
                            {
                                maxFlightId = point.FlightId;
                            }
                            if(point.ClusteringLevel > maxLevel)
                            {
                                maxLevel = point.ClusteringLevel;
                            }
                        }
                        int levelsSpaces = Math.Round(maxLevel,2).ToString().Length > 5 ? maxLevel.ToString().Length : 5;

                        // Generate Column Labels
                        int digitsInMaxFlightId = maxFlightId.ToString().Length;
                        var columnTitles = new char[digitsInMaxFlightId, cluster.Points.Count()];
                        for(int i=0; i < digitsInMaxFlightId; i++)
                        {
                            sb.Append(new String(' ', levelsSpaces+4));
                            for(int j = 0; j < cluster.Points.Count(); j++)
                            {
                                var point = cluster.Points[j];
                                var precedingSpaces = digitsInMaxFlightId - point.FlightId.ToString().Length;

                                if (precedingSpaces > 0)
                                {
                                    if (i < precedingSpaces) {
                                        columnTitles[i, j] = ' ';
                                        sb.Append("  ");
                                    } else
                                    {
                                        columnTitles[i, j] = point.FlightId.ToString()[i - precedingSpaces];
                                        sb.Append(point.FlightId.ToString()[i - precedingSpaces] + " ");
                                    }
                                } else
                                {
                                    columnTitles[i, j] = point.FlightId.ToString()[i - precedingSpaces];
                        ...
                        sb.Append("\n");
                        for(int m=0; m < levelsSpaces-5; m++)
                        {
                            sb.Append(" ");
                        }
                        sb.Append("LEVEL    ");
                        // append level ordering
                        foreach(var point in cluster.Points)
                        {
                            sb.Append(point.ClusteringOrder + " ");
                        }
                        sb.Append("\n");
                        sb.Append(new String('-', levelsSpaces));
